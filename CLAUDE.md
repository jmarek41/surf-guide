# Surf Guide — Claude Code context
...
- `data/` is private and gitignored. Rider profiles, exact bases, board history,
  listing shortlists, raw session logs, and personal calibration belong there.
...
- Never reveal secrets, unpublished breaks, private seller messages, exact
  accommodation addresses, or personally identifying session details.

## Quality rules

- For factual spot, board, listing, or forecast claims, prefer two independent
  sources. If that is not available, label the claim `single-source` or
  `unverified`; never fill a field by guessing.
...
- `.claude/skills/` — user-facing workflows.
